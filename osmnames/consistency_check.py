...
def missing_parent_ids():
    ...
    missing_points = count("SELECT COUNT(id) FROM osm_point WHERE merged IS FALSE AND parent_id IS NULL")
    if missing_points > 0:
        log.warning('{} points (not merged) with missing parent_id'.format(missing_points))
