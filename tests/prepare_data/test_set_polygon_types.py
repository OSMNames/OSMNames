...
def test_polygon_type_is_updated_correctly_when_mapping_override_exists(session, tables):
    # Polygons use negative osm IDs but the mapping override table stores them postively
    # so this test needs to use negative osm IDs in the polygon table otherwise it wont match
    session.add(tables.osm_polygon(id=1, osm_id=-2094141, country_code='nz', admin_level=6, type='administrative'))
