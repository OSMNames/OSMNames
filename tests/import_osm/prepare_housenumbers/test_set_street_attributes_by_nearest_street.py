...
def test_do_not_search_street_if_street_name_is_already_set(session, schema, tables):
    session.add(
            tables.osm_housenumber(
                    id=1,
                    osm_id=195916994,
    ...
    assert str(session.query(tables.osm_housenumber).get(1).street) == "Some Street"
